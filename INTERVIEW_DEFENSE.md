...
**Answer:** In priority order: secrets out of appsettings into a vault/env injection, asymmetric JWT keys or delegating auth to an identity provider, and migrations as a deploy step. (Transactional outbox, idempotent consumers, persistent inventory, a CI pipeline, and distributed tracing used to top this list; all five are implemented now — see Q8, Q9a, Q18/Q19a, Q19b, and the GitHub Actions workflow.) I kept the rest out deliberately — the README says so — because a junior project that *knows* its cuts beats one that pretends to be Netflix.
